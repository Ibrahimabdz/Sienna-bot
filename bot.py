...
async def on_member_join(member: discord.Member):
    ...
    log = discord.Embed(title="📥 Nouveau membre", color=0x57F287, timestamp=datetime.utcnow())
    ...
    log.add_field(name="🎂 Compte créé", value=f"<t:{int(member.created_at.timestamp())}:R>", inline=True)
    log.add_field(name="👥 Total membres", value=str(guild.member_count), inline=True)
    if inviter:
        log.add_field(name="✉️ Invité par", value=f"{inviter.mention} (`{inviter.id}`)", inline=True)
        log.add_field(name="🔗 Code invitation", value=f"`{invite_code}`", inline=True)
    else:
        log.add_field(name="✉️ Invité par", value="Inconnu", inline=True)
