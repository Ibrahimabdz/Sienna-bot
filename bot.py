...
def load_data():
    ...
    try:
        ...
        BUMP_CHANNEL_ID       = cfg.get("BUMP_CHANNEL_ID",       BUMP_CHANNEL_ID)
        BUMP_ROLE_ID          = cfg.get("BUMP_ROLE_ID",          BUMP_ROLE_ID)
        bump_enabled          = cfg.get("bump_enabled",          bump_enabled)
...
async def slash_confession(interaction: discord.Interaction, texte: str):
    ...
    # Supprime la commande du salon où elle a été tapée pour cacher qui l'a envoyée
    try:
        await interaction.channel.purge(limit=1, check=lambda m: m.author.id == bot.user.id)
    except Exception:
        pass
