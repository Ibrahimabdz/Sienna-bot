...
async def on_member_join(member: discord.Member):
    guild = member.guild
    # Auto-rôles (peut en donner plusieurs)
    ...
    if ch:
        if WELCOME_GIF_ENABLED:
            ...
            embed = discord.Embed(
                title=f"⚔️ Bienvenue dans la Taverne, {member.display_name} !",
                description=(
                    f"Les portes s'ouvrent pour {member.mention} ! 🍺\n\n\n📜 **Avant de commencer :**\n\n┣ ✅ Accepte les **règles**\n\n┗ 🎭 Choisis tes **rôles** !"
                ),
                color=0x8B0000, timestamp=datetime.utcnow()
            ...
            embed.set_footer(text=f"🍻 Aventurier #{guild.member_count}")
            await ch.send(embed=embed)
    # Log
    log = discord.Embed(title="📥 Membre rejoint", color=0x57F287, timestamp=datetime.utcnow())
    log.set_thumbnail(url=member.display_avatar.url)
    log.add_field(name="Membre", value=f"{member.mention} (`{member.id}`)")
    log.add_field(name="Compte créé le", value=f"<t:{int(member.created_at.timestamp())}:D>")
    log.add_field(name="Total membres", value=str(guild.member_count))
...
async def on_message_delete(message: discord.Message):
    ...
    embed.add_field(name="Auteur", value=f"{message.author.mention} (`{message.author.id}`)")
    if message.content:
...
async def on_message_edit(before: discord.Message, after: discord.Message):
    ...
    embed.add_field(name="Auteur", value=f"{before.author.mention} (`{before.author.id}`)")
    ...
    if now - xp_cooldowns.get(uid, 0) >= XP_COOLDOWN:
        ...
        if not leveled:
            ...
            if LEVELUP_GIF_ENABLED and lvlup_ch:
                ...
                # Fallback : message dans le salon actuel
                embed = discord.Embed(
                    title="⬆️ Level Up !",
                    description=f"Félicitations {message.author.mention}, tu passes au **niveau {lvl}** ! 🎉",
                    color=0xFFD700, timestamp=datetime.utcnow()
...
@tree.command(name="rank", description="Affiche ton niveau et ton XP")
...
async def slash_rank(interaction: discord.Interaction, membre: discord.Member = None):
    ...
    embed  = discord.Embed(title=f"📊 Rang de {target.display_name}", color=0xFFD700)
    embed.set_thumbnail(url=target.display_avatar.url)
    embed.add_field(name="Niveau",      value=str(data["level"]))
...
# ════════════════════════════════════════════════════════════════
#  COMPTEURS DE MEMBRES (salons vocaux auto-mis à jour)
# ════════════════════════════════════════════════════════════════
