...
CONFESSION_CHANNEL_ID: int = 0   # salon où envoyer les confessions
confession_counter:    int = 0   # numéro auto-incrémenté

class ConfessionModal(discord.ui.Modal, title="🤫 Confession Anonyme"):
    contenu = discord.ui.TextInput(
        label="Ta confession",
        style=discord.TextStyle.paragraph,
        placeholder="Écris ta confession ici... Elle sera envoyée anonymement.",
        min_length=5,
        max_length=1000,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not ch:
            await interaction.response.send_message("❌ Salon de confession introuvable.", ephemeral=True)
            return
...
class ConfessionView(discord.ui.View):
    """Bouton pour ouvrir le modal de confession."""
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(
        label="🤫 Faire une confession",
        style=discord.ButtonStyle.secondary,
        custom_id="open_confession"
    )
    async def open_confession(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(ConfessionModal())


@tree.command(name="confessionpanel", description="[Admin] Envoie le panel de confession anonyme")
@app_commands.describe(salon="Salon où envoyer le panel (bouton)")
@app_commands.checks.has_permissions(administrator=True)
async def slash_confessionpanel(interaction: discord.Interaction, salon: discord.TextChannel = None):
    ch = salon or interaction.channel
    embed = discord.Embed(
        title="🤫 Confessions Anonymes",
        description=(
            "Tu as quelque chose à dire mais tu n'oses pas ?\n\n"
            "Clique sur le bouton ci-dessous pour envoyer une **confession totalement anonyme**.\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "🔒 Ton identité ne sera **jamais** révélée.\n"
            "📜 Les confessions sont envoyées dans le salon dédié.\n"
            "⚠️ Les confessions inappropriées peuvent entraîner des sanctions."
        ),
        color=0x2C2F33
    )
    embed.set_footer(text="La Taverne • Confession anonyme")
    await ch.send(embed=embed, view=ConfessionView())
    await interaction.response.send_message(f"✅ Panel confession envoyé dans {ch.mention} !", ephemeral=True)
