...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    # ── Détection Boost ────────────────────────────────────────
    boosted   = not before.premium_since and after.premium_since
    unboosted = before.premium_since and not after.premium_since

    if boosted:
        # Cherche le salon boost configuré
        ch = after.guild.get_channel(BOOST_CHANNEL_ID) if BOOST_CHANNEL_ID else None
        if not ch:
            # Fallback : salon système du serveur
        ...
        if ch:
            ...
            embed = discord.Embed(
                description=(
                    ...
                    f"Merci pour ton soutien, aventurier ! 🍺\n"
                    f"Grâce à toi, la Taverne compte maintenant **{boost_count} boost(s)** "
        ...
        log.add_field(name="Boosted par", value=f"{after.mention} (`{after.id}`)")
